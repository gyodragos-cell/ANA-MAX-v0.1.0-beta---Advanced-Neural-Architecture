...
def html_to_text(html: str, max_chars: int = 8000) -> str:
    """Convert HTML into compact ASCII-safe text."""
    payload = str(html or "")
    payload = re.sub(r"(?is)<(script|style|noscript).*?>.*?</\1>", " ", payload)
    payload = re.sub(r"(?is)<br\s*/?>", " ", payload)
    payload = re.sub(r"(?is)</p\s*>", " ", payload)
    payload = re.sub(r"(?is)<[^>]+>", " ", payload)
    payload = html_lib.unescape(payload)
    payload = " ".join(payload.split())
    return _ascii_text(payload[: max(0, int(max_chars or 0))])


def _default_fetcher(url: str, timeout: int) -> tuple[str, str, int]:
    request = Request(
        url,
...
def web_scrape(
    ...
    """Fetch http/https HTML and return compact text for OS-22 RAG ingestion."""
    target = str(url or "").strip()
    valid, error = _validate_http_url(target)
    if not valid:
        return {
            "schema": "ana.os22.web_scrape.v1",
            "success": False,
            "local_only": True,
            "url": target,
            "text": "",
    ...
    try:
        html_payload, content_type, status_code = (fetcher or _default_fetcher)(target, int(timeout or 30))
        text = html_to_text(html_payload, max_chars=max_chars)
        return {
            "schema": "ana.os22.web_scrape.v1",
            "success": True,
            "local_only": True,
            ...
            "truncated": len(text) >= int(max_chars or 0),
            "error": "",
        }
