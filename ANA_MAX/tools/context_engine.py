...
_learned_patterns: dict         = {}   # key a' {count, confidence, next_action, last_seen}
_last_notify: dict              = {}   # intent_key a' timestamp (cooldown)
...
def apply_feedback(pattern_key: str, accepted: bool) -> dict:
    ...
    try:
        ...
        if accepted:
            ...
            msg = f"Confidenta crescuta: {old_c:.2f} a' {p['confidence']:.2f}"
        else:
            p["confidence"] = max(MIN_CONFIDENCE, old_c - FEEDBACK_PENALTY)
            p["rejected"]   = p.get("rejected", 0) + 1
            msg = f"Confidenta scazuta: {old_c:.2f} a' {p['confidence']:.2f}"
