...
class ANAAgent:
    ...
    def _init_ollama(self, model_override: Optional[str] = None) -> None:
        ...
        self._ollama_model = model_override or config.get("ai.ollama.model", "qwen2.5:7b")
    ...
    def send_message(self, message: str, allow_auto_tools: bool = True) -> str:
        ...
        # Daca toate routele sunt in cooldown, resetam cooldown-ul pentru a nu bloca sistemul
        now = time.time()
        all_in_cooldown = all(r.cooldown_until > now for r in self._routes)
        if all_in_cooldown:
            logger.info("Toate backend-urile sunt in cooldown. Resetez cooldown-urile pentru a incerca din nou.")
        ...
        last_error_text = str(last_error) if last_error is not None else "unknown error"
        if not last_error_text:
            last_error_text = type(last_error).__name__ if last_error is not None else "unknown error"
        return f"Eroare la procesarea mesajului: {last_error_text}"
    ...
    def _send_ollama(self, message: str) -> str:
        """Send a message through Ollama using /api/chat with ANA tool calling."""
        from core.backends import ollama_backend
        return ollama_backend.send(self, message)
